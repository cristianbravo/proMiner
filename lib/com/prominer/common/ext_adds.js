/* 
 * Hacks y adds para el framework ext.
 */
//Validacion del RUT via vtype
Ext.apply(Ext.form.VTypes,{
    'rutText': "RUT no válido.",
    'rutRe': /^0*(\d{1,3}(\.?\d{3,6}\.?)*)\-?([\dkK])$/,
    'rut': function (v) {
		var r = v.split('-');
        ...
        return this.rutRe.test(v) && dv(r[0]) == r[1] ? true : false;
    }
});


Ext.apply(Ext.form.VTypes,{
     'unsigned': function(val, field){
         try{
     ...
     'unsignedText': 'Numeros enteros!', //mensaje de error
     'unsignedMask': / /  //regexp para filtrar los caracteres permitidos
 }
);
//Validacion del email via vtype
Ext.apply(Ext.form.VTypes,{
    'emailText': "Email no valido.",
...
//manejo dinamico de show/hide de componentes en formulario
